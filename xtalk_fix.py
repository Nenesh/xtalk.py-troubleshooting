...
model_size = "medium" # Supported model : English models: tiny.en, base.en, small.en, medium.en . Multilingual Models: tiny, base, small, medium, large, large-v2, large-v3.
whisper_model = WhisperModel(model_size, device="cuda", compute_type="float16") # Can add "vad_filter=True" if problem with voice detection.

# Set up xtts
language_xtts = "en" # Supported language : English (en), Spanish (es), French (fr), German (de), Italian (it), Portuguese (pt), Polish (pl), Turkish (tr), Russian (ru), Dutch (nl), Czech (cs), Arabic (ar), Chinese (zh-cn), Japanese (ja), Hungarian (hu) and Korean (ko) 
tts_speed = 1.2 # Specify the desired speed.
audio_file_pth2 = "./XTTS-v2/samples/en_sample.wav" # Add your reference voice (the voice you want for the bot) in XTTS-v2/samples and specify the path. Exemple : "./XTTS-v2/samples/melissa_voice.wav" .
